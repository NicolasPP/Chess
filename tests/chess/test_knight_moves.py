...
def test_base_moves():
    assert len(GAME.get_available_moves('KNIGHT', 36, FEN.Fen('8/8/8/8/8/8/8/8 w KQkq - 0 1'), True)) is 8
...
def test_default_fen(from_index: int, is_white_turn: bool):
    ...
    assert len(GAME.get_available_moves('KNIGHT', from_index, fen, is_white_turn)) == 2


def test_fully_blocked():
    assert len(GAME.get_available_moves('KNIGHT', 36, FEN.Fen('8/8/3p1p2/2p3p1/4k3/2p3p1/3p1p2/8 b KQkq - 0 1'), False)) == 0


def test_jump_over_piece():
    assert(len(GAME.get_available_moves('KNIGHT', 36, FEN.Fen('8/8/3P1P2/2PpppP1/3pkp2/2PpppP1/3P1P2/8 b KQkq - 0 1'), False)) == 4)
