...
def test_base_moves():
    assert len(get_available_moves('R', 36, Fen('8/8/8/8/8/8/8/8 b KQkq - 0 1'))) == 14
...
def test_default_fen(from_index: int, is_white_turn: bool):
    assert len(get_available_moves('R', from_index, Fen(), is_white_turn)) is 0


def test_fully_blocked():
    assert len(get_available_moves('R', 36, Fen('8/8/8/4p3/3prp2/4p3/8/8 b KQkq - 0 1'))) is 0


def test_possible_take():
    fen = Fen('8/8/8/4P3/3PrP2/4P3/8/8 b KQkq - 0 1')
    moves = get_available_moves('R', 36, fen)
...
def test_blocked_take():
    assert len(get_available_moves('R', 36, Fen('8/8/4P3/4p3/2PprpP1/4p3/4P3/8 b KQkq - 0 1'))) is 0
