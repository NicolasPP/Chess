...
def test_base_moves():
    assert len(GAME.get_available_moves('ROOK', 36, FEN.Fen('8/8/8/8/8/8/8/8 b KQkq - 0 1'), False)) == 14
...
def test_default_fen(from_index: int, is_white_turn: bool):
    ...
    assert len(GAME.get_available_moves('ROOK', from_index, fen, is_white_turn)) is 0


def test_fully_blocked():
    assert len(GAME.get_available_moves('ROOK', 36, FEN.Fen('8/8/8/4p3/3prp2/4p3/8/8 b KQkq - 0 1'), False)) is 0
...
def test_blocked_take():
    assert len(GAME.get_available_moves('ROOK', 36, FEN.Fen('8/8/4P3/4p3/2PprpP1/4p3/4P3/8 b KQkq - 0 1'), False)) is 0
